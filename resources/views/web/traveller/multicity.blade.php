...
<section class="flight-summery">
    <div class="container">
        ...
        <form name="traveller_details" id="traveller_details" method="POST" action="{{url('/multicity-traveller-details')}}">
                    @csrf
        ...
        <div class="row">
            <div class="col-lg-8">
                ...
                <?php $c = 0;
                foreach ($booking_details['tripInfos'] as $row) {
                ?>
                    <div class="white-card">
                        <div class="row">
                            <?php
                            $start_tym = explode("T", $row['sI'][0]['dt']);
                            $end_tym = explode("T", $row['sI'][0]['at']);
                            ...
                            <div class="col-lg-12">
                                <div class="Departure-flight-summery">
                                    <p class="flight-summery-heading">{{$row['sI'][0]['da']['city']}} - {{$row['sI'][0]['aa']['city']}}</p>
                                    <p class="font18">Departure {{ date_format($final_startdate,"D, d M y") }}</p>
                                </div>
                            </div>
                            @php
                            $logo_details = DB::table('airlines')->where('airlines_code',$row['sI'][0]['fD']['aI']['code'])->where('airline_status',1)->first();
                            @endphp
                            ...
                            <div class="col-lg-4 d-flex align-items-center align-self-start">
                                ...
                                <p class="jet-name">{{$row['sI'][0]['fD']['aI']['name']}}</p>
                            </div>
                            <?php
                            if ($c == 0) {
                            ...
                            <div class="col-lg-3">
                                ...
                                <p class="dept-city">{{ $row['sI'][0]['da']['city'] }}
                                    <?php if (!empty($row['sI'][0]['da']['terminal'])) {
                                        echo $row['sI'][0]['da']['terminal'];
                                    }
                                    ?>
                                </p>
                            ...
                            <div class="col-lg-2 text-center">
                                <p class="timing">{{ hoursandmins($row['sI'][0]['duration']) }}</p>
                            ...
                            <div class="col-lg-3">
                                ...
                                <p class="dept-date">{{ date_format($final_startdate,"D, d M y") }}</p>
                                <p class="dept-city">{{ $row['sI'][0]['aa']['city'] }}</p>
                                <p class="teminal-no">{{ $row['sI'][0]['aa']['city'] }}-{{ $row['sI'][0]['aa']['countryCode'] }},
                                    <?php if (!empty($row['sI'][0]['aa']['terminal'])) {
                                        echo $row['sI'][0]['aa']['terminal'];
                                    }
                                    ?>
                                </p>
                            </div>
                        ...
                        <?php
                        if (!empty($fareRules[$c]['policy'])) : ?>
                            <h3 class="flight-summery-heading">Fare Rules</h3>
                            <div class="col-lg-12">

                                <p><b>Policy</b><br>{{$fareRules[$c]['policy']}}</p>
                                <p><b>Date Change Policy</b><?php echo str_replace('__nls__', '<br>', $fareRules[$c]['dateChangePolicy']); ?></p>
                                <p><b>Cancellation Policy</b><?php echo str_replace('__nls__', '<br>', $fareRules[$c]['cancellationPolicy']); ?></p>

                            </div>
                        <?php endif; ?>
                        <?php if (!empty($row['sI'][0]['ssrInfo']['BAGGAGE'])) : ?>
                            <h3 class="flight-summery-heading">Extra Baggages</h3>
                            <div class="white-card">
                                <div class="col-lg-12">

                                    <?php foreach ($row['sI'][0]['ssrInfo']['BAGGAGE'] as $row1) {
                                        // echo '<pre>'; var_dump($row);
                                    ?>
                                        <input type="checkbox" class="baggage" id="{{$row1['amount']}}" name="baggages[{{$c}}][]" value="{{$row1['code']}}"> Pay Extra ₹{{$row1['amount']}} - {{$row1['desc']}}<br>
                                    <?php
                                    }
                                    ?>
                                </div>
    ...
    <?php if (!empty($row['sI'][0]['ssrInfo']['MEAL']) && $row['totalPriceList'][0]['fd']['ADULT']['mI'] == true) : ?>
        <h3 class="flight-summery-heading">Select Free meals</h3>
        <div class="white-card">
            <div class="col-lg-12">

                <?php foreach ($row['sI'][0]['ssrInfo']['MEAL'] as $row1) {
                    // echo '<pre>'; var_dump($row);
                ?>
                    <input type="radio" name="meals[{{$c}}][]" value="{{$row1['code']}}"> {{$row1['desc']}}<br>
                <?php
                }
                ?>
            </div>
    ...
    <?php if (!empty($row['sI'][0]['ssrInfo']['MEAL']) && $row['totalPriceList'][0]['fd']['ADULT']['mI'] == false) : ?>
            ...
            <div class="col-lg-12">
                ...
                ?>
                    &nbsp; <?php if ($row1['amount'] == 0) {
                                echo "Free Provided";
                            } else { ?><input type="checkbox" name="meals[{{$c}}][]" class="meal" value="{{$row1['code']}}" id="{{$row1['amount']}}"> <?php echo "Pay Extra ₹" . $row1['amount'];
                                                                                                                                                                                        } ?> - {{$row1['desc']}}<br>
                <?php
                }
                ?>
