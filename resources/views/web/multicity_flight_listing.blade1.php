<?php //echo '<pre>'; var_dump($all_flightdata['searchResult']['tripInfos']['RETURN']); die; ?>
...
?>
        @if(count($all_flightdata['searchResult']['tripInfos']))
        <?php for($i=0; $i<count($all_flightdata['searchResult']['tripInfos']); $i++) { 
            $in=$i;
            
            if(count($all_flightdata['searchResult']['tripInfos'])==2){
                if($i==0) { $in='ONWARD';}
                if($i==1) { $in='RETURN'; }
            }
            if(count($all_flightdata['searchResult']['tripInfos'])==1){
                $in='ONWARD'; 
            }
            

            // die(count($all_flightdata['searchResult']['tripInfos'])."va;ir".$in);
            // echo '<pre>'.$in; var_dump($all_flightdata['searchResult']['tripInfos'][$in]); die;    


            ?>
            @foreach($all_flightdata['searchResult']['tripInfos'][$in] as $dt_key =>  $dt)
                <?php
                ...
                ?>
                <div class="card-item {{ $dt['sI'][0]['fD']['aI']['code'] }}code {{ $dt['sI'][0]['stops'] }}stops" data-flight-price-range="{{$dt['price_range']}}" data-flight-min-price="{{$dt['min_price']}}" data-flight-departure="{{$start_strtotime }}" data-flight-arrival="{{$end_strtotime }}" data-flight-duration="{{ $dt['sI'][0]['duration'] }}" id="unique-{{ $dt['totalPriceList'][0]['id'] }}">
                                                    ...
                                                    <li class="nav-item">
                                                        ...
                                                        <a class="" id="one-way-tab{{$x}}" data-toggle="tab" href="#one-way{{$x}}" role="tab" aria-controls="one-way" aria-selected="true">
                                                            <?php 
                                                               ...
                                                               ?>
                                                        <input type="radio" name="{{$in}}" value="{{$tpl['id']}}">
